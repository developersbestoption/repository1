...
<body>
     <form method="post" action="rollnumber.php"  id="rollnumberform"  enctype="multipart/form-data">
  ...
  <div class="row">
     ...
     <span id="RequiredFieldValidator1" style="color:Red;visibility:hidden;">Please enter Roll Number</span>
           <p id="place"> </p>
              ...
              <div class="col-md-6"><p id="place"></p>
...
if (isset($_POST["Button1"])) {
    $numberToCheck = $_POST["roll_txtbx"];

    require "conection.php";

    // Query to check if the number exists in the database
    $query = "SELECT * FROM basicdata WHERE rollno = $numberToCheck";
    $result = $connection->query($query);
      $row = mysqli_fetch_assoc($result);
      $roll = $row['rollno'];
      if ( $numberToCheck == $roll) {

         echo '<script>

        document.getElementById("place").innerHTML="it is present in database";
        </script>';
       echo "Number is present in the database.";
   }
   // else
  //   {
  ...
  //  // $connection->close();
  echo '<script>
  document.getElementById("place").innerHTML="it is not present in database";
  </script>';}
?>
</body>
</html>
